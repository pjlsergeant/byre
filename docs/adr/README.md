...
- 0047: the project mount can plant host-run code: byre exit-reports noticed changes, never gates; annotated -- the tools BYRE itself spawns resolve through one pinned resolver and decline a binary resolved out of a box-writable directory (engine = hard refusal, the report's own git probes = degrade + one disclosure, and an engine ENUMERATION never reads declined as absent -- totals commands refuse; "/" is never a root: a project AT the filesystem root loses the check by accepted residual -- the user's explicit choice to hand the box their whole writable surface; the pre-fix alternative declined every binary outright) [arm: TestExitReportGitConfig, TestLookDeclinesShadowedBinary, TestNewRootsDropsFilesystemRoot, TestBoxWritableRootsForFilesystemRootCwd, TestDevelopRefusesShadowedEngine, TestDevelopDisclosesShadowedGitAndContinues, TestInstalledEnginesReportsDeclinedSeparately, TestInstalledEnginesDeclinesRelativePathEntry, TestLifecycleEnginesRefusesOverADeclinedEngine]
- 0048: the four accretion guardrails are standing commitments in PRINCIPLES.md, unnumbered under the principles [no arm]
- 0049: a compat path lives two minors or 90 days from replacement, whichever is longer; the last supported release warns, then it is removed whole with a remedy; a LIVE key gets judgment instead of a window (RELEASING.md); amended 2026-08-23 -- while byre has no external users the calendar and warning-release step collapse: a compat path warns-and-keeps-working (file + fix named) and removal is a per-path operator call once its warnings go quiet; removal-release requirements unchanged; the amendment expires with the first external user [no arm]
...
- 0057: project credentials are confidentiality at rest + consent per launch, and NOTHING else -- values age-encrypted INLINE in the config files themselves (an `encrypted:`/`encrypted-file:` env_from_host row per value, encrypted to the recipient in that file's own file-local [credentials] block, whose identity is scrypt-wrapped; a row is the cascade-visible consent to the value, and setting one never prompts because the recipient is the cleartext half), the passphrase as the per-launch consent act (three attempts per contributing file, root-most first, each answer tried against every still-locked identity); credentials are BLOCKING -- every unlock or decrypt failure stops the launch naming file, row and remedy, and `--credentials=skip` is the one deliberate way to launch without them; delivery over exec-stdin onto the session tmpfs with the manifest before the `.done` sentinel and a bounded FAIL-CLOSED launcher wait (0011's shape, not its opposite: no sentinel, a manifest line the launcher cannot honor, or a bare restart, and the agent never runs); byre's plaintext lives in exactly one filesystem place (the tmpfs, an 0052 managed path whose durable shadow can re-surface and re-export on bare restart -- disclosed, not gated); in-box code, user mounts, and the config files themselves are NOT defended (the payload key/kind stamps are accident guards, not integrity; a config writer is the disclosed residual); the scrypt unwrap runs pre-lock, per-row decrypts run under the setup lock, every write is a compare-and-swap on the whole file under the lock that guards THAT file -- the FILE owns the lock and every cooperating writer takes it (the project's setup lock, or a layer's own lock, which the `--layer` editor and `byre layer new` take too), rekey rotates the passphrase only and leaves every value blob byte-identical, every pre-launch read is bounded; a value NEVER rides argv; the record carries unlock + decrypt outcomes ("scheduled", never "delivered") and there is NO live-state surface; stderr says plain "delivered" only when the inject provably landed inside the launcher's wait; the preset bridge (a hostile repo writer can MINT/SWAP/TRANSPLANT/REPLAY a value through `preset apply`) is answered at the consent gate, not by storage -- the review annotates any changed row where EITHER side is a credential scheme ("if you didn't rotate this credential, reject" -- either-side so ciphertext-to-plaintext cannot dodge it), rows that appeared or vanished more quietly, and any change to the file-local [credentials] block, judged over both files' RAW bytes with ciphertext elided [arm: TestInlineRoundtrip, TestInlineRekeyLeavesValueBlobsByteIdentical, TestInlineKeyBindingMismatch, TestEncryptedRowsAreFileLocal, TestCredentialsSetMintsTheIdentityAndWritesTheRow, TestCredentialWritesCompareAndSwap, TestLayerCredentialWritesTakeTheLayerLock, TestLayerNewTakesTheLayerLock, TestDevelopDeliversCredentials, TestDevelopNonTTYStopsWithRemedies, TestDevelopWrongPassphraseStopsTheLaunch, TestReceiverWritesValuesAndDoneLast, TestReceiverRefusesACredentialKeyedManifest, TestLauncherExportsEnvAndFileKinds, TestLauncherCredWaitFailsClosed, TestLauncherManifestRejectionsFailClosed, TestLauncherCredExportWinsEnvdCollision, TestDockerRunCarriesTheCredentialGate, TestInjectDeadlineUnderLauncherWait, TestCredDeliveredLineHonesty, TestManagedPathShadows, TestLauncherRestartWithoutRedeliveryRefuses, TestInjectFailureStopsTheBoxAndFailsTheLaunch, TestCredentialReviewFlagsAChangedValue, TestCredentialReviewFlagsEitherSide, TestCredentialReviewFlagsBlockChanges, TestCredentialReviewIsSilentWithoutAChange, TestPresetReviewCarriesTheCredentialAnnotation]
- 0058: `develop --agent` (`-a`) on a configured project is a RUN-SCOPED override -- resolved exactly as if the config's `agent` key said so (skill enabled implicitly even when not in the config, CLI/state volume/egress/adapters on the normal paths; installed packages only, the install-hint error stands), REPLACE semantics (an implicitly-enabled config agent drops out of the run; ride-alongs via `skills` compose as ever), NOTHING written and NO shared auth (companions enter via the written config only -- structural, and pinned by test); the override survives the under-lock re-read, `--agent none` runs agentless and a BLANK value is rejected naming the sentinel remedy, an already-running session gets the no-effect notice, and `byre worktree -a` forwards the flag to its handoff develop as the OVERRIDE only (a never-developed repo refuses it by name rather than letting onboarding write it repo-wide); the launch record carries `agent` + `agent_override` with agentless launches written as the "none" sentinel (empty = pre-agent record ONLY, which leaves the row config-derived) and status renders the running box's agent with the override/drift qualifier; `--template`/`--shared-auth` stay first-run-only [arm: TestAgentOverrideResolvesLikeAWrittenKeyWithoutWriting, TestAgentOverrideNoneRunsAgentless, TestAgentOverrideBlankValueIsRejected, TestAgentOverrideUnknownSkillFailsNamingIt, TestOnboardExistingConfigWithFlagErrors, TestWorktreeHandoffAgentRefusesUnconfiguredProject, TestWorktreeHandoffAgentProceedsOnConfiguredProject, TestLaunchRecordCapturesWhatTheEngineWasTold, TestLaunchRecordAgentlessWritesTheNoneSentinel, TestStatusAgentRowSpeaksForTheLaunchedBox]
