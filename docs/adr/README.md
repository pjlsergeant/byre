...
- 0058: `develop --agent` (`-a`) on a configured project is a RUN-SCOPED override -- resolved exactly as if the config's `agent` key said so (skill enabled implicitly even when not in the config, CLI/state volume/egress/adapters on the normal paths; installed packages only, the install-hint error stands), REPLACE semantics (an implicitly-enabled config agent drops out of the run; ride-alongs via `skills` compose as ever), NOTHING written and NO shared auth (companions enter via the written config only -- structural, and pinned by test); the override survives the under-lock re-read, `--agent none` runs agentless, an already-running session gets the no-effect notice; the launch record carries `agent` + `agent_override` and status renders the running box's agent with the override/drift qualifier (a pre-agent record leaves the row config-derived); `--template`/`--shared-auth` stay first-run-only [arm: TestAgentOverrideResolvesLikeAWrittenKeyWithoutWriting, TestAgentOverrideNoneRunsAgentless, TestAgentOverrideUnknownSkillFailsNamingIt, TestOnboardExistingConfigWithFlagErrors, TestLaunchRecordCapturesWhatTheEngineWasTold, TestStatusAgentRowSpeaksForTheLaunchedBox]
