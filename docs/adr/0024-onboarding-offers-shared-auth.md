...
- **Readiness is declared, not inferred.** A companion skill says
  ...
  gate -- grok-shared-auth is RETIRED (failed its field gate, ADR 0023)
  and gemini's OAuth path is gate-pending, so neither declares the key
  and neither is offered, while gemini's stays hand-enableable by
  someone who has read its skill.toml. Two skills claiming one agent is refused
...
companion skill.toml keeps its pre-0024 content until updated -- the
standard pickup path for every shipped skill change). When gemini's
OAuth gate passes or a grok rebuild passes its gates (the parked v2
designs, ADR 0023), declaring `shared_auth_for` in their skill.toml is
the entire rollout of their offer.
