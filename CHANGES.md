...
- **Config-family reads are fd-judged and bounded; a planted FIFO or symlink
  ...
  path record, preset review snapshots, a repo-side `byre.preset`) now opens
  through `internal/hostopen`: non-regular files and oversize reads fail
  loudly with the path named. The one case a user may notice: a **symlink**
  planted as a store project config now refuses -- replace the symlink with
  the real file and re-run. Host-owned homes are deliberately exempt: a
  dotfiles-symlinked `~/.byre/default.config` (and named layers) keeps
  working -- the loader follows symlinks exactly where no agent can reach.
  An AST conformance test enforces the rule tree-wide from here on.
- **Help vocabulary matches the glossary: box and session, not container.**
  The root, develop, status, and shell help now say "box"/"session state";
  engine-level rows keep "container". One registrar now builds the `skill`
  and `template` subtrees, so a fix lands on both nouns by construction; the
  site commands page regenerated byte-identical apart from the wording
  above.
- **`docs/PLACEMENT.md` holds the placement principles** (P1-P11, numbering
  intact) extracted from the marketing doc; ADR 0048 promotes the accretion
  guardrails into PRINCIPLES.md, ADR 0049 records the compat-retirement
  windows and inventory, and `wip/config-complexity-todo.md` is absorbed
  and deleted.
