...
func TestDevelopRefusesWhenSessionLive(t *testing.T) {
	...
	err := develop(f, io.Discard, p, config.Config{}, skills.Resolved{}, false)
...
func TestDevelopBuildsSeedsThenRuns(t *testing.T) {
	...
	f := &fakeRunner{}
	if err := develop(f, io.Discard, p, cfg, skills.Resolved{}, false); err != nil {
		t.Fatal(err)
	}
...
func TestDevelopSelfEditNotesAndMount(t *testing.T) {
	...
	if err := develop(f, &stderr, p, config.Config{}, skills.Resolved{}, true); err != nil {
		t.Fatal(err)
	}
