...
// own comment forms. Anchored: a bare Contains would claim any file that
// merely mentions the phrase as byre-owned and overwrite it. Every
// generation, legacy included, has begun a header comment this way.
...
func installLinux(s Streams, a appInstall, d installDeps) error {
	...
	if cfg, _, err := image.DecodeConfig(bytes.NewReader(png)); err == nil {
		...
		if err := hostopen.PlainMkdirAll(iconDir, 0o755, hostopen.HostUserOwned); err == nil {
			...
			if existing, err := hostopen.PlainReadFile(iconPath, hostopen.HostUserOwned); err == nil &&
			...
			} else if err := hostopen.PlainWriteFile(iconPath, png, 0o644, hostopen.HostUserOwned); err == nil {
				fmt.Fprintf(s.Out, "%s\n", iconPath)
			}
		}
	}
