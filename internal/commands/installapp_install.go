...
func installDarwin(s Streams, box string, d installDeps) error {
	...
	if _, err := os.Stat(appPath); err == nil {
		if prev, err := os.ReadFile(marker); err != nil || !strings.Contains(string(prev), "Generated by byre") {
	...
	if err := d.run("osacompile", "-o", appPath, srcPath); err != nil {
	...
	if err := os.WriteFile(marker, []byte(source), 0o644); err != nil {
		return err
	}
	...
	if icns, err := packICNS(deliverIconPNG); err == nil {
		for _, name := range []string{"droplet.icns", "applet.icns"} {
			_ = os.WriteFile(filepath.Join(appPath, "Contents", "Resources", name), icns, 0o644)
		}
	} else {
	...
	if err := d.run("codesign", "--force", "--deep", "--sign", "-", appPath); err != nil {
	...
	// The Finder Quick Action (right-click → Deliver to Byre).
	...
	if prev, err := os.ReadFile(wflowPath); err == nil && !strings.Contains(string(prev), "byre") {
...
func installLinux(s Streams, box string, d installDeps) error {
	...
	if prev, err := os.ReadFile(entryPath); err == nil && !strings.Contains(string(prev), "Generated by byre") {
