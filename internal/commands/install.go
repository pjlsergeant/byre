...
func pkgInstall(s Streams, kind packages.Kind, uri, expectDigest string, yes bool) error {
	...
	// with the remedy rather than manufacturing a conflict row.
	...
	// Same ID, same digest: no-op (D9a).
	if replacing && old.Digest == acq.Digest {
...
func pkgUninstall(s Streams, kind packages.Kind, id string, yes bool) error {
	...
	if !ok {
		idx, ierr := packages.ReadIndex(home)
		if ierr == nil {
			if row, present := idx[cat.ExpandAlias(id)]; present {
				// Broken installed row (snapshot missing etc.) -- still removable.
				ent = &packages.Entry{ID: cat.ExpandAlias(id), Kind: packages.Kind(row.Kind), Provenance: packages.ProvInstalled}
				ok = true
			}
		}
		if !ok {
