...
func commonGitDirOf(paths project.Paths) string {
	...
	fi, err := hostopen.StatNoFollow(dir)
	if err != nil || !fi.IsDir() {
		return ""
	}
	return dir
...
func gitConfigFiles(gitDir string) (fixed, fromListing []string, listed bool) {
	...
	entries, err := hostopen.ReadDirNoFollow(filepath.Join(gitDir, "worktrees"))
	if err != nil {
...
func envFiles(workDir string) ([]string, bool) {
	entries, err := hostopen.ReadDirNoFollow(workDir)
	if err != nil {
		return nil, false
	}
	var out []string
