...
func TestAssembleContextTargetWithoutSkillContext(t *testing.T) {
	...
	for _, key := range []string{"apt =", "npm_global", "dockerfile_pre", "dockerfile_post", "run_args", "skills =", "mounts =", "volumes =", "byre.config"} {
		if !strings.Contains(string(doc), key) {
			t.Errorf("self-edit doc should reference %q:\n%s", key, doc)
		}
	}
	if strings.Contains(string(doc), "ports") {
		t.Errorf("self-edit doc must not document a nonexistent 'ports' key:\n%s", doc)
	}
}
