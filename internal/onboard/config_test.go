...
func TestSaveSharedAuthDefaultYesCreatesFileAndList(t *testing.T) {
	home := t.TempDir()
	if err := SaveSharedAuthDefault(home, "claude", "claude-shared-auth", true); err != nil {
		t.Fatal(err)
	}
...
func TestSaveSharedAuthDefaultYesAppendsPreservingContent(t *testing.T) {
	home := t.TempDir()
...
func TestSaveSharedAuthDefaultArrayShapes(t *testing.T) {
...
func TestSaveSharedAuthDefaultIdempotent(t *testing.T) {
	home := t.TempDir()
...
func TestSaveSharedAuthDefaultRefusesUnparsableFile(t *testing.T) {
	...
	err := SaveSharedAuthDefault(home, "claude", "claude-shared-auth", true)
...
func TestSaveSharedAuthDefaultNoRecordsDecline(t *testing.T) {
	home := t.TempDir()
	writeDefault(t, home, "agent = \"claude\"\n")
	if err := SaveSharedAuthDefault(home, "claude", "claude-shared-auth", false); err != nil {
		t.Fatal(err)
	}
	cfg, err := config.ParseFile(filepath.Join(home, "default.config"))
	if err != nil {
...
func TestSharedAuthAnswered(t *testing.T) {
	...
	if SharedAuthAnswered(home, "claude", "claude-shared-auth") {
		t.Fatal("no file yet — no machine default settles the question")
	}
	writeDefault(t, home, "skills = [\"claude-shared-auth\"]\n")
	if !SharedAuthAnswered(home, "claude", "claude-shared-auth") {
		t.Fatal("companion enabled machine-wide = answered yes")
	}
	writeDefault(t, home, "shared_auth_declined = [\"claude\"]\n")
	if !SharedAuthAnswered(home, "claude", "claude-shared-auth") {
		t.Fatal("saved decline = answered no")
	}
	writeDefault(t, home, "skills = [\"devloop\"]\n")
	if SharedAuthAnswered(home, "claude", "claude-shared-auth") {
		t.Fatal("other skills enabled ≠ answered")
	}
	// An unparsable file counts as answered: never nag through (or, on a
	// save, edit) a file we can't read.
