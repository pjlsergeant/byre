...
# opencode-shared-auth's own link into the identity volume is legitimate,
# and a DANGLING one is its expected first-login state (opencode writes
# in place, VERIFIED through a symlink 2026-07-16 — the login writes
# through it into the shared volume). Canonicalize the target's PARENT dir
# (the final auth.json may be absent); a lexical prefix check would accept
# planted ..-traversals and reject legitimate relative links. Relative
# targets resolve from the link's own directory.
shared_auth=""
if [ -L "$cred" ]; then
  target="$(readlink "$cred")"
  tdir="$(cd "$data_root/opencode" 2>/dev/null && cd "$(dirname "$target")" 2>/dev/null && pwd -P)" || tdir=""
  case "$tdir/" in
  "${BYRE_IDENTITY_BASE:-/home/dev/.byre-identity}"/*) shared_auth=1 ;;
  *) rm -f "$cred" ;;
  esac
fi
...
# {"type":"oauth",...} — the binary's own Auth schema). This catches the
# empty/truncated artifacts an interrupted login could leave (which a bare
# -s test would wrongly count as logged in) but not a server-side-expired
# credential — that surfaces at use time, where the fix is the same
# command: `opencode auth login`.
[ -s "$cred" ] && grep -q '"type"' "$cred" 2>/dev/null && exit 0
