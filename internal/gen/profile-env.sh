...
# login shell. Sourcing them into every login shell is safe and quiet ONLY
# because hooks honor the ADR 0028 purity contract: the exported environment is
# a hook's only lasting effect (anything that DOES something lives in
# firstrun.d, run by the launcher only). There is no mechanism behind that here
# -- a login shell has no strict-mode belt at all, so the contract is the whole
# protection. The BYRE_ENVD_DIR override mirrors the launcher's test seam.
