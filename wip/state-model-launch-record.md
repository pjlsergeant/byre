# The missing "box that was actually launched" -- review findings, verified

**Status: OPEN, undesigned.** Four robustness findings from an external code
review (relayed by Pete, 2026-07-28), each verified CONFIRMED against HEAD the
same day with the evidence below. Nothing here is decided; remedy shapes are
the reviewer's proposals plus session notes. TODO.md carries the item; delete
this file when the work ships or the decisions land in ADRs.

These are all correctness/robustness observations about byre's own behavior:
status output describing the wrong thing, config edits landing in the wrong
order, byre resolving a host tool by bare name, and a volume-sharing assumption
that no longer holds. None of it is security-testing work; it is defensive
product work, and the ratified plan is `wip/state-model-and-status-plan.md`.
...
Edit the config while a box runs and status shows the next-launch values
...
## 2. Saves and launches are not ordered relative to each other -- CONFIRMED
...
resolves a config carrying a mount; the editor saves the removal under the lock
and reports success; develop acquires the lock and launches the stale
resolution, mount included. Same shape in rebuild and worktree. The editor's
drift check protects the editor from the file changing under it, not develop
from a stale read.
...
## 3. Host tool resolution runs whatever PATH points at -- CONFIRMED
...
Nothing pins a resolved path or checks it against directories the box can write
(hostopen governs file I/O, not exec resolution). So if the host PATH lists an
ABSOLUTE directory that the box can write (direnv `.venv/bin`, a project
`.bin`) ahead of the real binary, and that directory contains a `git`/`docker`,
byre resolves and runs it host-side instead of the intended system tool --
automatically, since the exit report's own probes fire at every session end.
Go's ErrDot handling already declines relative PATH entries, so the case that
needs fixing is the absolute one.

The doctrine gap is precise: ADR 0047 covers the case where a person later runs
a tool over the project tree; the case where byre ITSELF runs a tool
automatically is not yet covered -- and the exit report added by 0047 is one
such automatic run.

Reviewer's fix (and the session's read: treat as the urgent, independent
piece): resolve absolutes before the box starts, decline binaries resolved from
directories the box can write, reuse the pinned paths for the process lifetime.
...
tolerates two concurrent writers without corrupting.
